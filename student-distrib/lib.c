...
void scroll_down()
{
	uint8_t video_tmp[NUM_COLS*NUM_ROWS*2];
	int32_t i;
	
	for(i=0;i<NUM_COLS*(NUM_ROWS-1)*2; i++)
		video_tmp[i] = video_mem[i+(NUM_COLS<<1)];
		
	for(i=0;i<NUM_COLS*(NUM_ROWS-1)*2; i++)
		video_mem[i]=video_tmp[i];
...
void scroll_up()
{
	...
	for(i=0;i<NUM_COLS*(NUM_ROWS-1)*2; i++)
		video_tmp[i] = video_mem[i];
		
	for(i=0;i<NUM_COLS*(NUM_ROWS-1)*2; i++)
		video_mem[i+(NUM_COLS<<1)]=video_tmp[i];
...
void update_screen_loc(int x, int y) {
	if(x<0) backspace(x,y);
	else if(x < NUM_COLS) previous_line(x,y);
	else next_line(x,y);
}
...
putc(uint8_t c)
{
    if(c == '\n' || c == '\r') {
        screen_y++;
        screen_x=0;
		update_screen_loc(screen_x, screen_y);
    } else {
...
*			uint32_t n = number of bytes to copy
*   Return Value: pointer to dest
*	Function: copy n bytes of the source command string into the destination string
*/

int8_t*
cmdncpy(int8_t* dest, const int8_t* src, uint32_t n)
{
	int32_t i=0;
	while(src[i] != ('\0' || ' ') && i < n) {
		dest[i] = src[i];
